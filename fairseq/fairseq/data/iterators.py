...
class EpochBatchIterator(EpochBatchIterating):
    ...
    # ordered_batches函数实现功能：根据epoch数的不同，对所有语料进行重新排序，这里得到的语料排序正是src_token的语料排序
    def ordered_batches(self, epoch, fix_batches_to_gpus, shuffle):
        # 打印发现，epoch值为3，表示当前是第3个epoch；fix_batches_to_gpus值为False；shuffle值为True

        def shuffle_batches(batches, seed):
            with data_utils.numpy_seed(seed):
                # print(f'%%%%%%%%%%%%%%%%%%:{batches[0]}')
                # print(f'the value of the seed is:{seed}')
                # 打印发现，if语句没有执行
                ...
                # 打印发现，else语句被执行,shuffle之前batches[0]的长度始终为816，shuffle之后batches[0]的长度始终为88
                # 因此通过设置seed，每次输入的数据都是相同的顺序
                # 通过对batches[i]的长度进行累加，发现值等于160250，说明是所有语料序号的展示
                # batches为一个列表，列表中的每个元素为一个ndarray，其内部的值表示语料的序号
                # 通过对batches打乱前后，batches内部的ndarray进行比较，发现只是打乱了ndarray的顺序，ndarray内部表示语料标签的顺序并没有打乱
                else:
        ...
        # 通过打印发现，训练时执行了下面的else语句
        else:
            # 通过打印发现，执行了下面的if语句
            if shuffle:
                # 传入的seed值随着epoch不断变化，因此每个epoch进入模型的语料顺序都是不同的
                batches = shuffle_batches(list(self.frozen_batches), self.seed + epoch)
            # 通过打印发现，没有执行下面的else语句
            else:
                batches = self.frozen_batches
            # 通过打印batches信息，没有发现经过下面语句前后，batches信息有发生变化
            # 函数返回的batches就是经过shuffle的语料顺序，按照序号进行排列
            batches = list(
                ShardedIterator(batches, self.num_shards, self.shard_id, fill_value=[])
            )
        return batches
